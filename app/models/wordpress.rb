...
class Wordpress < ActiveRecord::Base
  ...
  def get_orders
    ...
    all_orders = woocommerce.get("orders?filter[limit]=50&filter[order]=asc&status=processing&fields=id,shipping_address,billing_address,line_items").parsed_response
